...
A Python library for conjugating verbs in French, English, Spanish, Italian, Portuguese and Romanian (mores soon).
                ...
                if isinstance(persons, list):
                    ...
                    for pers, term in persons:
                        if len(persons) == 6:
                            ...
                            key = IMPERATIVE_PRONOUNS[self.language][self.subject][pers] + ' ' + NEGATION[self.language]
                        else:
                            key = pers
                        if term is not None:
                            persons_dict[key] = self.verb_info.root + term
